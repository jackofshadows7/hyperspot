...
mod tests {
    ...
    fn test_parse_orderby_too_long() {
        ...
        assert!(result.unwrap_err().contains("orderby too long"));
    }
    ...
    fn test_parse_orderby_too_many_fields() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("too many order fields"));
    }
    ...
    fn test_parse_orderby_invalid_clause() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("invalid orderby clause"));
    }
    ...
    async fn test_extract_odata_query_full() {
        ...
        assert_eq!(message, "ORDER_WITH_CURSOR");
    ...
    async fn test_extract_odata_query_filter_too_long() {
        ...
        let (status, message) = result.unwrap_err();
        assert_eq!(status, StatusCode::BAD_REQUEST);
        assert!(message.contains("filter too long"));
    }
    ...
    async fn test_extract_odata_query_invalid_orderby() {
        ...
        let (status, message) = result.unwrap_err();
        assert_eq!(status, StatusCode::BAD_REQUEST);
        assert!(message.contains("invalid $orderby"));
    }
